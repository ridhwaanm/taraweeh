...
async function seed() {
  ...
  try {
    for (const recording of sampleRecordings) {
      const hafidhId = await getOrCreateHafidh(recording.hafidh);
      const venueId = await getOrCreateVenue(recording.venue, recording.city);

      await insertRecording.run(
        hafidhId,
        venueId,
        recording.year,
        recording.url,
      ...
      console.log(
        `✓ Added: ${recording.hafidh} at ${recording.venue}, ${recording.city}`,
      );
    }
